...
class OPTIMIZE:
    ...
    def __init__(
        ...
        rd_sel = np.random.choice(np.arange(0, self.n_at),
                                  size=self.ncg,
                                  replace=False)
        for el in rd_sel:
            self.mapping[el] = 1
    ...
    def make_a_move(self):
        ...
        self.mapping[at1] = 0
        self.mapping[at2] = 1
        print(f"move made: removing {at1}, inserting {at2}")
    ...
    def append_quantities(self, hs, hk, smap, smap_inf):
        """Append quantities to self.cg_mappings."""
        self.cg_mappings.append([self.ncg,
                                 self.atom_ret.copy(),
    ...
    def run(self, df):
        ...
        for _n in range(self.nsteps):
            ...
            hs_t, hk_t, smap_t, smap_inf_t = self.calculate_observables(df)
            accepted = self.metropolis(smap, smap_t)
            if accepted:
                smap = smap_t
                hs = hs_t
                hk = hk_t
